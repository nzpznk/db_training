...
void load(const char* tree_file)
{
	freopen(tree_file,"r",stdin);
	tree.load();
...
void construct_gtree(const char* edge_file, const char* node_file) {
	...
	tree.build();
	cout << "G-Tree build finished" << endl;
}
...
int network_dist(int ori_id, int dst_id) {
	return tree.search(ori_id, dst_id);
}

int find_path(int ori_id, int dst_id, vector<int>& path) {
	return tree.find_path(ori_id, dst_id, path);
}
